    ...
    <div className="w-full h-fit flex flex-col justify-between gap-2 max-w-72 relative align-top select-none">
      <div className="relative w-full h-full flex flex-col justify-between gap-2 align-top ">
        ...
        <div className="w-full flex justify-center">
          <Image
            src={image}
            className={`mask mask-decagon h-36 w-2/3 self-center ${cover ? "object-cover" : "object-contain"}`}
            draggable={false}
            width={500}
            height={500}
            alt="name"
        ...
        <div className="flex flex-col gap-2 z-10 min-h-48 max-lg:min-h-64 justify-between">
          <div className="flex flex-col gap-2 w-full">
            <p className="text-2xl font-bold text-HSWaccent text-center max-lg:text-lg">
              {name}
            </p>
          ...
          <div className="bg-HSWaccent flex gap-2 w-full p-2 justify-center">
            {cybersoc && (
              <Link
                href={
                  "https://www.linkedin.com/company/exeter-cyber-security-society/posts/?feedView=all"
                }
                target="_blank"
                className="text-center justify-center rounded-full w-8 h-8 bg-HSWsecondary cursor-pointer p-[2px]"
              >
                <Image
            ...
            {github !== "" && (
              <Link
                href={github}
                target="_blank"
                className="text-center justify-center rounded-full w-8 h-8 bg-HSWsecondary cursor-pointer"
              >
                <VscGithubAlt className="w-full h-full p-1" />
              </Link>
            )}
            {linkedin !== "" && (
              <Link
                ...
                className="text-center justify-center rounded-full w-8 h-8 bg-HSWsecondary cursor-pointer"
              >
                <CiLinkedin className="w-full h-full p-1" />
              </Link>
            )}
            {website !== "" && (
              <Link
                ...
                className="text-center justify-center rounded-full w-8 h-8 bg-HSWsecondary cursor-pointer"
              >
                <IoIosLink className="w-full h-full p-1" />
              </Link>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2 z-10 min-h-48 max-lg:min-h-64 justify-between">
          ...
          <div className="bg-HSWaccent flex gap-2 w-full p-2 justify-center">
            <Link
              ...
              href={github}
              target="_blank"
              className="text-center justify-center rounded-full w-8 h-8 bg-HSWtext cursor-pointer transition-all duration-200"
              onMouseEnter={() => {
                console.log("enter");
                // set the timer
                ...
                if (centerX + x > window.innerWidth)
                  x = window.innerWidth - 100;
                if (centerY + y > window.innerHeight)
                  y = window.innerHeight - 100;

                div.style.transform = `translate(${x}px, ${y}px)`;
              }}
            >
              <VscGithubAlt className="w-full h-full p-1 animate-pulse text-HSWprimary" />
            </Link>
            {linkedin !== "" && (
              <Link
                href={linkedin}
